...
export default class Grid extends Taro.Component {
  ...
  renderComponent(num) {
    return generatorArr(num).map((un, index) => (
      <View key={Math.random()}>
        <ClCard type='full' bgColor={index % 2 === 0 ? 'light-blue' : 'light-brown'}>{index + 1}</ClCard>
      </View>
    ))
  }
  ...
  render () {
    return (
      <ClLayout>
        <ClTitleBar title='等分列' textColor='blue' type='icon' subTitle='grid' subTitleColor='shadow' />
        {this.renderCards(1)}
        {this.renderCards(2)}
        {this.renderCards(3)}
        {this.renderCards(4)}
        ...
        <ClTitleBar title='等高' textColor='blue' type='icon' subTitle='square' subTitleColor='shadow' />
        <ClCard>
