...
import { ClButton, ClLayout, ClTitleBar, ClAnimation, ClCard } from 'mp-colorui'
  ...
  const delay = animationName.map((name, index) => (
    <ClLayout key={name} margin='normal' marginDirection='around'>
      <ClAnimation type={`${begin ? 'scale-up' : ''}`} delay={index/10}>
        <ClButton long shape='round' bgColor='brown' onClick={() => {beginAnimation(index)}} text={`${index ? index/10 + 's' : '开始执行'}`} />
      </ClAnimation>
    </ClLayout>
  ))
  return (
    <ClLayout>
      <ClTitleBar type='icon' title='默认效果' iconColor='red' textColor='black' />
      {showNormal}
      <ClTitleBar type='icon' title='反向效果' iconColor='red' textColor='black' />
      {showReserve}
      <ClTitleBar type='icon' title='延迟执行' iconColor='red' textColor='black' />
      {delay}
    </ClLayout>
  )
}
